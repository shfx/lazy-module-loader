...
class Context {
  ...
  restore(module) {
    ...
    if (lastModule !== module) {
      throw new Error(
        `Invalid context detected: '${lastModule.id}', expecting: ${module.id}`
      );
    }
  }
...
class Loader {
  ...
  async load(module) {
    ...
    try {
      ...
      const exportPromise = isBrowser
        ? this.loadInBrowser(path)
        : this.loadInNode(path);
